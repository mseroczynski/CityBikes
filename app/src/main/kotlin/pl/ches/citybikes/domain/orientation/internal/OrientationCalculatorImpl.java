...
import pl.ches.citybikes.domain.orientation.internal.math.Util;
import pl.ches.citybikes.domain.orientation.internal.math.Vector3;
import pl.ches.citybikes.domain.orientation.internal.math.Vector4;

/**
 * A really brute-force implementation to derive the orientation of the device based on where the back of the device is
 * pointing when not held flat, or, when held flat, where the top of the device is pointing. This is tricky, because
 * where the device is pointing changes depending on how we're holding the device, as well as the screen rotation of the
 * device.
 *
 * What this does is rotate all the points according to the rotation matrix, and then take a look at where reference
 * points ended up (6 points on a sphere, to determine bearing, pitch, roll). Yeah, it's bad, and there has to be a much
 * simpler way.
 *
...
public class OrientationCalculatorImpl implements OrientationCalculator {
  ...
  public void getOrientation(Matrix4 rotationMatrix, int screenRotation, float[] out) {
    ...
    if (Math.abs(deviceAltitude) < 75) {
      ...
      deviceBearing = Util.floatrev((angle + ((angleIncrement
          * Math.cos(Util.calcAngleClamp(distToR, Util.calcRadius(distToL, distToR, distLR)))
          * distToL) / distLR)) - 180);
    } else {
    ...
    if (Math.abs(deviceAltitude) < 75) {
      ...
      float adist = (float) Math.sqrt(
          ((upDown.x - mOriginPoint.x) * (upDown.x - mOriginPoint.x)) + ((upDown.y - mOriginPoint.y) * (upDown.y
              - mOriginPoint.y)));
      float bdist = (float) Math.sqrt(
          ((upDown.x - topBot.x) * (upDown.x - topBot.x)) + ((upDown.y - topBot.y) * (upDown.y - topBot.y)));
      float cdist = (float) Math.sqrt(
          ((mOriginPoint.x - topBot.x) * (mOriginPoint.x - topBot.x)) + ((mOriginPoint.y - topBot.y) * (mOriginPoint.y
              - topBot.y)));

      float val = (((adist * adist) + (cdist * cdist)) - (bdist * bdist)) / (2 * adist * cdist);
      if (val < 1) {
        float atheta = (float) Math.acos((((adist * adist) + (cdist * cdist)) - (bdist * bdist)) / (2 * adist * cdist))
            * RADIANS_TO_DEGREES;
        if (altAbove) {
